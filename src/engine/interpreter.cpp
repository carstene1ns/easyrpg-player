...
#include "engine/interpreter_control_variables.h"
...
void Game_Interpreter::Push(
	...
	if (_state.stack.empty() && main_flag && !Game_Battle::IsBattleRunning()) {
		...
		Main_Data::game_hero->SetMenuCalling(false);
		Main_Data::game_hero->SetEncounterCalling(false);
	}
...
void Game_Interpreter::Update(bool reset_loop_count) {
	...
	if (Input::IsTriggered(Input::DEBUG_ABORT_EVENT) && Player::debug_flag && !Game_Battle::IsBattleRunning()) {
		...
		if (!Main_Data::game_hero->GetMoveRoute().move_commands.empty()) {
	...
	for (; loop_count < loop_limit; ++loop_count) {
		...
		if (main_flag) {
			if (Main_Data::game_hero->IsBoardingOrUnboarding())
				break;

			if (Main_Data::game_hero->InVehicle() && Main_Data::game_hero->GetVehicle()->IsAscendingOrDescending())
				break;
...
bool Game_Interpreter::CommandChangeSpriteAssociation(lcf::rpg::EventCommand const& com) { // code 10630
	...
	Main_Data::game_hero->ResetGraphic();
	return true;
}
...
bool Game_Interpreter::CommandSetVehicleLocation(lcf::rpg::EventCommand const& com) { // code 10850
	...
	if (Main_Data::game_hero->GetVehicle() == vehicle) {
		if (map_id == Game_Map::GetMapId()) {
			if (vehicle) {
				vehicle->MoveTo(map_id, x, y);
			}
			Main_Data::game_hero->MoveTo(map_id, x, y);
			if (vehicle_id == 0) {
				...
				Main_Data::game_hero->SetJumping(false);
			}
			return true;
		};
