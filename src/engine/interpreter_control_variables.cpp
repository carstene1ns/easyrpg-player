...
#include "engine/interpreter_shared.h"
		...
		switch (op) {
			...
			case 4: {
				...
				if (Player::game_config.fake_resolution.Get()) {
					int pan_delta = (Game_Hero::GetDefaultPanX() - lcf::rpg::SavePartyLocation::kPanXDefault) / TILE_SIZE;
			...
			case 5: {
				// Screen Y
				if (Player::game_config.fake_resolution.Get()) {
					int pan_delta = (Game_Hero::GetDefaultPanY() - lcf::rpg::SavePartyLocation::kPanYDefault) / TILE_SIZE;
