...
#include "engine/interpreter_map.h"
...
void Game_Map::SetupFromSave(
	...
	const bool is_db_save_compat = Main_Data::game_hero->IsDatabaseCompatibleWithSave(lcf::Data::system.save_count);
	const bool is_map_save_compat = Main_Data::game_hero->IsMapCompatibleWithSave(GetMapSaveCount());
...
void Game_Map::PlayBgm() {
	...
	if ((current_info->ID > 0) && !current_info->music.name.empty()) {
		...
		if (!Main_Data::game_hero->IsAboard()) {
...
bool Game_Map::CanEmbarkShip(Game_Hero& hero, int x, int y) {
	auto bit = GetPassableMask(hero.GetX(), hero.GetY(), x, y);
	return IsPassableTile(&hero, bit, hero.GetX(), hero.GetY());
}

bool Game_Map::CanDisembarkShip(Game_Hero& hero, int x, int y) {
	...
	int bit = GetPassableMask(x, y, hero.GetX(), hero.GetY());
...
void Game_Map::UpdateProcessedFlags(bool is_preupdate) {
	...
	if (!is_preupdate) {
		Main_Data::game_hero->SetProcessed(false);
		for (auto& vehicle: vehicles) {
...
int Game_Map::GetMapId() {
	return Main_Data::game_hero->GetMapId();
}
...
std::vector<int> Game_Map::GetEncountersAt(int x, int y) {
	int terrain_tag = GetTerrainTag(Main_Data::game_hero->GetX(), Main_Data::game_hero->GetY());
...
static void OnEncounterEnd(BattleResult result) {
	...
	if (tt.IsActive()) {
		Main_Data::game_hero->ReserveTeleport(tt.GetMapId(), tt.GetX(), tt.GetY(), tt.GetDirection(), tt.GetType());
	}
}

bool Game_Map::PrepareEncounter(BattleArgs& args) {
	int x = Main_Data::game_hero->GetX();
	int y = Main_Data::game_hero->GetY();
...
void Game_Map::RemoveAllPendingMoves() {
	const auto map_id = GetMapId();
	Main_Data::game_hero->CancelMoveRoute();
	for (auto& vh: vehicles) {
