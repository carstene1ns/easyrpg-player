...
#ifndef EP_Game_Hero_H
#define EP_Game_Hero_H
...
class Game_Vehicle;
using Game_HeroBase = Game_CharacterDataStorage<lcf::rpg::SavePartyLocation>;

/**
 * Game Hero class
 */
class Game_Hero : public Game_HeroBase {
public:
	Game_Hero();
	...
	bool TriggerEventAt(int x, int y, bool triggered_by_decision_key, bool face_hero);

	/**
	 * Sets the map, position and direction that the game hero must have after the teleport is over
	 *
...
private:
	...
	bool CheckEventTriggerHere(TriggerSet triggers, bool triggered_by_decision_key, bool face_hero = true);
	bool CheckEventTriggerThere(TriggerSet triggers, int x, int y, bool triggered_by_decision_key, bool face_hero = true);
	bool GetOnVehicle();
	bool GetOffVehicle();
	bool UpdateAirship();
...
inline bool Game_Hero::IsPendingTeleport() const {
	return teleport_target.IsActive();
}

inline TeleportTarget Game_Hero::GetTeleportTarget() const {
	return teleport_target;
}

inline void Game_Hero::ResetTeleportTarget(TeleportTarget tt) {
	teleport_target = std::move(tt);
}

inline void Game_Hero::SetMenuCalling(bool value) {
	data()->menu_calling = value;
}

inline bool Game_Hero::IsMenuCalling() const {
	return data()->menu_calling;
}

inline void Game_Hero::SetEncounterCalling(bool value) {
	data()->encounter_calling = value;
}

inline bool Game_Hero::IsEncounterCalling() const {
	return data()->encounter_calling;
}

inline int Game_Hero::GetTotalEncounterRate() const {
	return data()->total_encounter_rate;
}

inline bool Game_Hero::IsPanActive() const {
	return GetPanX() != GetTargetPanX() || GetPanY() != GetTargetPanY();
}

inline bool Game_Hero::IsPanLocked() const {
	return data()->pan_state == lcf::rpg::SavePartyLocation::PanState_fixed;
}

inline int Game_Hero::GetPanX() const {
	return data()->pan_current_x;
}

inline int Game_Hero::GetPanY() const {
	return data()->pan_current_y;
}

inline int Game_Hero::GetTargetPanX() const {
	return data()->pan_finish_x;
}

inline int Game_Hero::GetTargetPanY() const {
	return data()->pan_finish_y;
}

inline bool Game_Hero::IsMapCompatibleWithSave(int map_save_count) const {
	return data()->map_save_count == map_save_count;
}

inline bool Game_Hero::IsDatabaseCompatibleWithSave(int database_save_count) const {
	return data()->database_save_count == database_save_count;
}

inline void Game_Hero::UpdateSaveCounts(int db_save_count, int map_save_count) {
	data()->database_save_count = db_save_count;
	data()->map_save_count = map_save_count;
}

inline bool Game_Hero::IsVisible() const {
	return !IsAboard() && Game_Character::IsVisible();
}

inline int Game_Hero::GetVehicleType() const {
	return data()->vehicle;
}

// FIXME: remove me soon
using Game_Player [[deprecated("Use Game_Hero")]] = Game_Hero;
