...
Game_Hero::Game_Hero(): Game_HeroBase(Hero)
{
...
void Game_Hero::SetSaveData(lcf::rpg::SavePartyLocation save)
{
	*data() = std::move(save);
...
lcf::rpg::SavePartyLocation Game_Hero::GetSaveData() const {
	return *data();
}

Drawable::Z_t Game_Hero::GetScreenZ(int x_offset, int y_offset) const {
...
void Game_Hero::ReserveTeleport(int map_id, int x, int y, int direction, TeleportTarget::Type tt) {
...
void Game_Hero::ReserveTeleport(const lcf::rpg::SaveTarget& target) {
...
void Game_Hero::PerformTeleport() {
...
void Game_Hero::MoveTo(int map_id, int x, int y) {
...
bool Game_Hero::MakeWay(int from_x, int from_y, int to_x, int to_y) {
	if (IsAboard()) {
...
void Game_Hero::MoveRouteSetSpriteGraphic(std::string sprite_name, int index) {
	auto* vh = GetVehicle();
	if (vh) {
...
void Game_Hero::UpdateScroll(int amount, bool was_jumping) {
	if (IsPanLocked()) {
		return;
	}
	...
	if (vehicle && vehicle->IsFlying()) {
		if (vehicle->AnimateAscentDescent()) {
			if (!vehicle->IsFlying()) {
				...
				Main_Data::game_hero->SetFlying(vehicle->IsFlying());
				data()->aboard = false;
				SetFacing(Down);
				data()->vehicle = 0;
...
void Game_Hero::UpdateNextMovementAction() {
	if (UpdateAirship()) {
		return;
	}
...
void Game_Hero::UpdateMovement(int amount) {
...
bool Game_Hero::CheckActionEvent() {
	if (IsFlying()) {
		return false;
	}
...
bool Game_Hero::CheckEventTriggerHere(TriggerSet triggers, bool triggered_by_decision_key, bool face_hero) {
	if (InAirship()) {
		return false;
	}
	...
	for (auto& ev: Game_Map::GetEvents()) {
		...
		if (ev.IsActive()
				...
				&& triggers[trigger]) {
			SetEncounterCalling(false);
			result |= ev.ScheduleForegroundExecution(triggered_by_decision_key, face_hero);
		}
	}
	return result;
}

bool Game_Hero::CheckEventTriggerThere(TriggerSet triggers, int x, int y, bool triggered_by_decision_key, bool face_hero) {
	if (InAirship()) {
		return false;
	}
	bool result = false;
...
bool Game_Hero::GetOnOffVehicle() {
...
void Game_Hero::ForceGetOffVehicle() {
	if (!IsAboard()) {
		return;
	}
...
bool Game_Hero::InVehicle() const {
	return data()->vehicle > 0;
}

bool Game_Hero::InAirship() const {
	return data()->vehicle == Game_Vehicle::Airship;
}

Game_Vehicle* Game_Hero::GetVehicle() const {
	return Game_Map::GetVehicle((Game_Vehicle::Type) data()->vehicle);
}

bool Game_Hero::Move(int dir) {
	if (!IsStopping()) {
		return true;
	}
...
bool Game_Hero::IsAboard() const {
	return data()->aboard;
}

bool Game_Hero::IsBoardingOrUnboarding() const {
	return data()->boarding || data()->unboarding;
}

void Game_Hero::UpdateEncounterSteps() {
...
void Game_Hero::StartPixelPan(int h, int v, int speed, bool interpolated, bool centered, bool relative) {
	if (!Player::IsPatchManiac()) {
		return;
	}
...
void Game_Hero::ResetPan(int speed) {
	data()->pan_finish_x = GetDefaultPanX();
	data()->pan_finish_y = GetDefaultPanY();
	data()->pan_speed = 2 << speed;
...
bool Game_Hero::TriggerEventAt(int x, int y, bool triggered_by_decision_key, bool face_hero) {
	return CheckEventTriggerThere({ lcf::rpg::EventPage::Trigger_action }, x, y, triggered_by_decision_key, face_hero);
}
