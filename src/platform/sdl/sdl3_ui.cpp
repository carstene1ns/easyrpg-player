...
static SDL_PixelFormat GetDefaultFormat() {
...
bool Sdl3Ui::HandleErrorOutput(const std::string &message) {
	...
	if ((current_display_mode.flags & SDL_WINDOW_FULLSCREEN)
		== SDL_WINDOW_FULLSCREEN) {
		SDL_SetWindowFullscreen(sdl_window, 0);
		SDL_SetWindowSize(sdl_window, 0, 0);
	}
...
void Sdl3Ui::ProcessKeyDownEvent(SDL_Event &evnt) {
#if defined(USE_KEYBOARD) && defined(SUPPORT_KEYBOARD)
	if (evnt.key.key == SDLK_F4 && (evnt.key.mod & SDL_KMOD_LALT)) {		// Close program on LeftAlt+F4
		Player::exit_flag = true;
		return;
	} else if (evnt.key.key == SDLK_RETURN ||
			evnt.key.key == SDLK_KP_ENTER) {
		if (evnt.key.mod & SDL_KMOD_LALT || (evnt.key.mod & SDL_KMOD_RALT)) {
	...
	// Update key state
	keys[SdlKey2InputKey(evnt.key.scancode)] = true;
#else
	/* unused */
	(void) evnt;
#endif
...
#if defined(USE_KEYBOARD) && defined(SUPPORT_KEYBOARD)
	keys[SdlKey2InputKey(evnt.key.scancode)] = false;
#else
	/* unused */
	(void) evnt;
#endif
...
void Sdl3Ui::ProcessMouseButtonEvent(SDL_Event& evnt) {
	...
	case SDL_BUTTON_LEFT:
		keys[Input::Keys::MOUSE_LEFT] = evnt.button.down;
		break;
	case SDL_BUTTON_MIDDLE:
		keys[Input::Keys::MOUSE_MIDDLE] = evnt.button.down;
		break;
	case SDL_BUTTON_RIGHT:
		keys[Input::Keys::MOUSE_RIGHT] = evnt.button.down;
		break;
	}
#else
	/* unused */
...
#if defined(USE_JOYSTICK) && defined(SUPPORT_JOYSTICK)
	int id = evnt.cdevice.which;
	SDL_Gamepad* controller = SDL_GetGamepadFromID(id);
	if (controller) {
...
#if defined(USE_JOYSTICK) && defined(SUPPORT_JOYSTICK)
	keys[SdlJKey2InputKey(evnt.gbutton.button)] = evnt.gbutton.down;
#endif
}
...
void Sdl3Ui::SetAppIcon() {
	...
	*/
	...
	#endif
	SDL_Surface *icon = SDL_CreateSurfaceFrom(ICON_SIZE, ICON_SIZE, SDL_GetPixelFormatForMasks(32, Rmask, Gmask, Bmask, Amask), icon32, ICON_SIZE*4);
...
bool Sdl3Ui::OpenURL(std::string_view url) {
	...
	if (!SDL_OpenURL(ToString(url).c_str())) {
