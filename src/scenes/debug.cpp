...
#include "windows/varlist.h"
#include "windows/numberinput.h"
#include "bitmap.h"
		...
		switch (mode) {
			...
			case eMoveSpeed:
				if (sz > 1) {
					...
					PushUiNumberInput(Main_Data::game_hero->GetMoveSpeed(), 1, false);
					range_index = 0;
					range_window->SetIndex(range_index);
				}
				break;
...
void Scene_Debug::DoMap() {
	...
	Scene::PopUntil(Scene::Map);
	if (Scene::instance) {
		Main_Data::game_hero->ReserveTeleport(map_id, x, y, -1, TeleportTarget::eSkillTeleport);
	}
}
...
void Scene_Debug::DoMoveSpeed() {
	Main_Data::game_hero->SetMoveSpeed(Utils::Clamp<int>(GetFrame().value, 1, 7));
