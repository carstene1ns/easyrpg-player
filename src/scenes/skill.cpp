...
#include "input.h"
#include "scenes/actortarget.h"
#include "scenes/teleport.h"
	...
	if (Input::IsTriggered(Input::CANCEL)) {
		...
		if (skill && skill_window->CheckEnable(skill_id)) {
			if (skill->type == lcf::rpg::Skill::Type_switch) {
				...
				Main_Data::game_system->SePlay(skill->sound_effect);
				Main_Data::game_party->UseSkill(skill_id, &actor, &actor);
				Main_Data::game_hero->ForceGetOffVehicle();
				Main_Data::game_hero->ReserveTeleport(Main_Data::game_targets->GetEscapeTarget());
