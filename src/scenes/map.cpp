...
void Scene_Map::Start2(MapUpdateAsyncContext actx) {
	...
	if (Main_Data::game_hero->IsPendingTeleport()) {
		TeleportParams tp;
		auto tt = Main_Data::game_hero->GetTeleportTarget().GetType();
...
void Scene_Map::UpdateStage2() {
	if (Main_Data::game_hero->IsPendingTeleport()) {
		const auto tt = Main_Data::game_hero->GetTeleportTarget().GetType();
		TeleportParams tp;
		tp.run_foreground_events = GetRunForegroundEvents(tt);
...
void Scene_Map::FinishPendingTeleport(TeleportParams tp) {
	...
	Main_Data::game_hero->PerformTeleport();
...
void Scene_Map::PerformAsyncTeleport(TeleportTarget original_tt) {
	Main_Data::game_hero->PerformTeleport();
	Main_Data::game_hero->ResetTeleportTarget(original_tt);
...
void Scene_Map::OnAsyncSuspend(F&& f, AsyncOp aop, bool is_preupdate) {
	...
	if (aop.GetType() == AsyncOp::eQuickTeleport) {
		...
		// the async teleport.
		auto orig_tt = Main_Data::game_hero->GetTeleportTarget();

		Main_Data::game_hero->ReserveTeleport(aop.GetTeleportMapId(), aop.GetTeleportX(), aop.GetTeleportY(), -1, TeleportTarget::eAsyncQuickTeleport);
